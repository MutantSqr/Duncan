...
def run_baseline_tests(project_root: Path, timeout: int = 120, pytest_args: Optional[List[str]] = None) -> BaselineResult:
    ...
    try:
        ...
        return BaselineResult(passed=False, returncode=-1, summary=summary, stdout=stdout, stderr=stderr)
    ...
    if proc.returncode != 0 and ("No module named pytest" in stderr or "ModuleNotFoundError: No module named 'pytest'" in stderr):
        ...
        return BaselineResult(passed=False, returncode=proc.returncode, summary=summary, stdout=stdout, stderr=stderr)
    ...
    return BaselineResult(passed=(proc.returncode == 0), returncode=proc.returncode, summary=summary, stdout=stdout, stderr=stderr)
